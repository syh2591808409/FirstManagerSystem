      ...
      <a-row :gutter="16">
        <a-col class="gutter-row" :span="16">
          <a-form-item
            ...
            <a-input size="large" v-model="randomvalidate" type="text" placeholder="验证码">
...
<!--            v-text="!state.smsSendBtn && '获取验证码'||(state.time+' s')"></a-button>-->
          <a href="javascript:void(0);" title="点击更换验证码">
            <img id="imgVerify" :src="randomvalidate_url" alt="更换验证码" height="36" width="100%" @click="getVerify">
          </a>
        </a-col>
      </a-row>
...
export default {
  ...
  data () {
    return {
      ...
      registerBtn: false,
      randomvalidate_url: 'http://127.0.0.1:9527/regist'
    }
  },
  computed: {
    passwordLevelClass () {
  ...
  methods: {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          this.$post('regist', {
            ...
            randomvalidate: this.randomvalidate
          }).then((r) => {
            if (r.data === true) {
              this.$message.success('注册成功')
              this.returnLogin()
            } else {
              this.$message.error('您输入的验证码不正确，请重新输入')
            }
          }).catch(() => {
    ...
    getVerify () {
      this.randomvalidate_url = 'http://127.0.0.1:9527/regist?' + Math.random()
    }
  }
}
</script>
