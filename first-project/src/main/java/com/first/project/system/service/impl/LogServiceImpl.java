...
import com.first.project.common.utils.AddressUtil;
import com.first.project.common.utils.SortUtil;
import com.first.project.system.dao.LogMapper;
...
public class LogServiceImpl extends ServiceImpl<LogMapper, SysLog> implements LogService {
    ...
    public IPage<SysLog> findLogs(QueryRequest request, SysLog sysLog) {
        try {
            ...
            SortUtil.handlePageSort(request, page, "createTime", FirstConstant.ORDER_DESC, true);
    ...
    @Override
    @Transactional
    public List<SysLog> findByLogId(String[] logIds){
        List<SysLog> sysLogList = new ArrayList<>();
        for(String logid:logIds){
            sysLogList.add(baseMapper.selectById(logid));
        }
        return sysLogList;
    }
